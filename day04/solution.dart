...
int findOverlapping(List<String> assignments, bool Function(List<List<int>> areas) overlapFilter) =>
    assignments.map(assignmentToAreas).map(areasToMinMax).where(overlapFilter).length;

num part1({String fileName = "input.txt"}) => findOverlapping(readFileToLines(fileName), areasOverlapFully);

num part2({String fileName = "input.txt"}) => findOverlapping(readFileToLines(fileName), areasOverlapPartial);
